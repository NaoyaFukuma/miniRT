...
/*   Updated: 2023/01/17 19:17:59 by kyamagis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ray	rt_eye_ray_constructor(t_3d_vec position_on_screen, t_rt_data *rt)
{
	...
	eyeRay.direction = rt_vector_sub(position_on_screen, CAMERA_POSITION);// 視線方向 pw - eyePos
	return (eyeRay);
}
...
t_rgb_vec	*rt_eye_raytrace(t_rt_data *rt, double fx, double fy)
{
	...
	eyeRay = rt_eye_ray_constructor(position_on_screen, rt);
	return (rt_raytrace(rt, eyeRay));
}
