...
/*   Updated: 2023/01/27 16:31:13 by nfukuma          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ray	rt_eye_ray_constructor(t_3d_vec position_on_screen, t_rt_data *rt)
{
	...
	eye_ray.start = rt_vec_add(rt->scene.cam.cam_p_vec,
			rt_vec_mult(rt->scene.cam.unit_cam_dir, epsilon));
	eye_ray.unit_d_vec = rt_vec_to_unit(rt_vec_sub(
				position_on_screen, rt->scene.cam.cam_p_vec));
	return (eye_ray);
}
