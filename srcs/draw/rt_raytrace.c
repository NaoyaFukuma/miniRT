...
t_intersection_testresult	*rt_malloc_intersection_testresult(t_obj *nearest_shape, t_intersection_point *nearest_intp)
{
	...
	while (objs)
	{
		...
		if (res != NULL && maxDist >= res->distance)
		{
			if (nearest_intp == NULL || nearest_intp->distance > res->distance)
			{
				...
				nearest_shape = objs;
				if (exitFound == true)
					return (rt_malloc_intersection_testresult(nearest_shape, nearest_intp));
			}
		}
		objs = objs->next;
	}
...
// t_intersection_testresult *rt_test_intersection_with_all_ray(t_obj *objs, t_ray ray, double maxDist, bool exitFound)
// {
// 	t_intersection_point	*nearest_intp = NULL; // 最も近い交点位置
// 	t_obj 					*nearest_shape = NULL; // 最も近い物体
