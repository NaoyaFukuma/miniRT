...
/*   Updated: 2023/02/02 13:00:40 by kyamagis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rt_fill_struct_pl(t_rt_data *rt, const char **tokens)
{
	...
	obj_ptr->plane->unit_norm_vec = rt_str_to_3dvector(tokens[2],
			-1, 1);
	...
	obj_ptr->plane->dx = rt_vec_to_unit(rt_vec_cross(
				obj_ptr->plane->unit_norm_vec, rt_vec_constructor(0, 1, 0)));
	if (rt_vec_mag(obj_ptr->plane->dx) == 0)
		obj_ptr->plane->dx = rt_vec_constructor(1, 0, 0);
	obj_ptr->plane->dy = rt_vec_cross(obj_ptr->plane->unit_norm_vec,
			obj_ptr->plane->dx);
}
