...
/*   Updated: 2023/01/30 11:07:50 by nfukuma          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_rgb_vec	rt_norm_tex_mapping(t_rt_data *rt, t_insec_p *res, t_sphere *sphere)
{
	...
	u = (1 - (phi / (2 * M_PI) + 0.5));
	v = (theta / M_PI);
	printf("1 unit_n_vec x[%f] y[%f] z[%f]\n", res->unit_n_vec.x,res->unit_n_vec.y,res->unit_n_vec.z);
	res->unit_n_vec = rt_bump_mapping(rt, res, u, v);
	printf("2 unit_n_vec x[%f] y[%f] z[%f]\n\n", res->unit_n_vec.x,res->unit_n_vec.y,res->unit_n_vec.z);
